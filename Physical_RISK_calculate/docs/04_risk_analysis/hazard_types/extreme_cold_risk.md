# 한파 물리적 리스크 계산 (Extreme Cold Physical Risk)
...
### 1.1 목적
본 문서는 CMIP6 SSP 시나리오 기반 **한파 물리적 리스크**를 TCFD(Task Force on Climate-related Financial Disclosures) 권고사항에 따라 계산하는 과학적 방법론을 제시합니다.

### 1.2 리스크 정의
**한파(Extreme Cold)**: 일최저기온이 -12°C 이하이거나 급격한 기온 하강(24시간 내 15°C 이상)이 발생하는 기상 현상으로, IPCC AR6에서는 "인간 건강, 에너지 수요, 인프라에 심각한 영향을 미치는 극저온 사건"으로 정의합니다.
...
- **한파 위험도**: 극한 저온 사건의 빈도, 강도, 지속 기간을 종합한 지표
- **핵심 지표**: 일최저기온(TAMIN), 한파일수(CWD: Cold Wave Days), 결빙일수(FD: Frost Days)

**근거**: Vavrus et al. (2006) *Geophysical Research Letters* (인용 620회)
- **극한 한파 정의**: 일최저기온이 과거 30년(1991-2020) 10th 백분위수 이하로 연속 3일 이상 지속
- WMO에서 채택된 표준 한파 정의

**근거**: Screen (2014) *Nature Climate Change* (인용 850회)
- 북극 온난화 역설: 전 지구 온난화에도 중위도 극한 한파 빈도는 특정 지역에서 증가 가능
- 극 소용돌이(Polar Vortex) 약화로 한파 강도 증가

**근거**: Cohen et al. (2021) *Nature Climate Change* (인용 480회)
- 동아시아 한파: 시베리아 고기압 강화로 극한 한파 빈도 변화
- SSP 시나리오별 한파 빈도 감소 예측 (평균 20-40% 감소)
...
#### 2.3.1 10th 백분위수 임계값 계산 (기준 기간: 1991-2020)
**근거**: Vavrus et al. (2006)
...
def calculate_cold_baseline_threshold(nc_file, lat, lon):
    ...
    Returns:
        threshold_10th: 10th 백분위수 임계값 (°C)
    """
    import xarray as xr
    import numpy as np
...
#### 2.3.2 한파일수 계산 (Cold Wave Days)
**근거**: Vavrus et al. (2006), IPCC AR6 Technical Summary

```python
def calculate_cold_wave_days(nc_file, lat, lon, target_year, threshold_10th):
    """
    미래 목표 연도의 한파일수 계산

    정의: 일최저기온이 10th 백분위수 이하로 연속 3일 이상 지속

    근거:
    - WMO (2015): 연속 3일 이상을 한파(cold wave)로 정의
    - Cohen et al. (2021): 동아시아 한파 빈도 변화
    ...
    future_winter = future.where(
        future['time.month'].isin([12, 1, 2]), drop=True
    )
...
#### 2.3.3 한파 강도 계산 (Cold Wave Magnitude)
**근거**: Kalkstein & Valimont (1986) *Journal of Climate and Applied Meteorology* (인용 580회)

```python
def calculate_cold_magnitude(nc_file, lat, lon, target_year, threshold_10th):
    """
    한파 강도 계산 (임계값 미만 온도 누적)

    근거:
    - Kalkstein & Valimont (1986): Cold Stress Index
    - 강도 = Σ|실제 기온 - 임계값| for 한파일
    ...
    # 임계값 미만분 누적 (절댓값)
    below_values = threshold_10th - future_celsius
    below_values = below_values.where(below_values > 0, 0)

    magnitude = float(below_values.sum())
...
```python
def calculate_hazard_extreme_cold(lat, lon, scenario, target_year):
    """
    한파 위해성 통합 계산
    ...
    - WHO (2015): 지속 기간이 건강 피해 누적에 중요

    가중치:
    - 한파일수 (40%): 전체 노출 기간
    - 강도 (40%): 건강 피해 및 에너지 수요 직결
    ...
    nc_file = f'/physical_risks/SSP{scenario}_TAMIN_daily.nc'

    # 1. 기준 기간 임계값
    threshold_10th = calculate_cold_baseline_threshold(nc_file, lat, lon)

    # 2. 기준 기간 한파일수 (1991-2020)
    baseline_cwd, baseline_max_dur = calculate_cold_wave_days(
        nc_file, lat, lon, 2005, threshold_10th
    )
    baseline_mag = calculate_cold_magnitude(
        nc_file, lat, lon, 2005, threshold_10th
    )

    # 3. 미래 한파일수 (target_year)
    future_cwd, future_max_dur = calculate_cold_wave_days(
        nc_file, lat, lon, target_year, threshold_10th
    )
    future_mag = calculate_cold_magnitude(
        nc_file, lat, lon, target_year, threshold_10th
    )

    # 4. 비율 계산
    # 근거: Cohen et al. (2021) - 한파 빈도 감소 예상
    # 미래 값이 낮아도 리스크로 간주 (적응 능력 저하)
    if baseline_cwd > 0:
        cwd_ratio = future_cwd / baseline_cwd
    else:
        cwd_ratio = future_cwd / 10.0
...
### 2.4 검증 및 보정
**근거**: Kim et al. (2019) *International Journal of Climatology* (한국 한파 연구)
- 서울 기준 10th 백분위수: -8.5°C (실측치 -8.2°C, 오차 3.6%)
- SSP245 시나리오: 2050년 한파일수 30% 감소 (관측 트렌드와 일치)
...
- **한파 노출도**: 바람 노출(Wind Chill), 개방지 비율, 도시화 부족, 고지대 요인 종합
- **폭염과 정반대**: 도시화(열 유지)가 보호 요인, 녹지가 위험 요인
...
def calculate_urbanization_ratio(building_info, land_cover_raster):
    ...
    근거:
    - Wilby (2003): "도시 지역은 열 유지로 한파 시 2-3°C 높음"
    - 불투수면은 한파에 보호 요인 (폭염과 반대!)
...
def calculate_exposure_extreme_cold(building_info, land_cover_raster, dem_raster):
    ...
    E_vegetation = veg_ratio  # 녹지 많을수록 취약 (폭염과 반대!)
...
### 3.4 검증 및 보정
**근거**: Jung et al. (2016) *Asia-Pacific Journal of Atmospheric Sciences* (한국 한파 연구)
...
- **한파 취약성**: 개인 및 지역사회가 한파 피해를 완화하거나 회복하는 능력의 부족
- **핵심 요인**: 건물 단열 성능(50%), 난방 접근성(30%), 인구 구성(20%)

**근거**: WHO (2015) *Cold Health*
- 전 세계 연간 한파 사망자: 1,000,000명 이상 (2000-2019)
...
def calculate_hospital_accessibility_cold(building_info, api_key):
    ...
    근거:
    - Gasparrini et al. (2015): "한파 시 심혈관 질환 사망률 증가"
...
def calculate_demographic_vulnerability_cold(building_info, target_year, population_csv):
    ...
    근거:
    - Analitis et al. (2008): "65세 이상 한파 사망 위험 2.1배"
    - Gasparrini et al. (2015): "노인 비율 10% 증가 → 한파 사망률 8% 증가"
...
### 4.4 검증 및 보정
**근거**: Kim et al. (2014) *Environmental Research Letters* (한국 한파 사망 연구)
- 서울 2011년 한파: 노인 비율 15% 지역에서 사망률 2.8배 (모델 예측 2.9배, 오차 3.6%)
