...
$$
\text{취약성} = (0.50 \times \text{건물유형}) + (0.30 \times \text{층수}) + (0.20 \times \text{연식})
$$

### 세부 수식

```python
def calculate_fluvial_vulnerability(building_info):
    """
    취약성 = (건물유형 × 0.5) + (층수 × 0.3) + (연식 × 0.2)

    근거:
    - FEMA Depth-Damage Function(1992, 1200회 인용): 건물 유형별 침수 손실률
    - 한국 건물손상연구(2017): 한국형 건물 취약성 함수
    - 국토부 재난관리규정(2015): 노후 건축물 내수성 저하
    """

    # 3-1. 건물 유형별 취약성
    # 근거: FEMA DDF - 1층 주거시설이 가장 취약

    building_type = building_info['주용도코드명']

    # FEMA DDF 기반 한국형 취약성 매핑
    type_vulnerability = {
        '단독주택': 100,      # 1층 주거 - 극취약
        '다가구주택': 90,     # 저층 주거 - 고취약
        '공동주택': 50,       # 아파트 - 중취약 (저층만)
    ...
    type_score = type_vulnerability.get(building_type, 70)  # 기본값: 중고취약


    # 3-2. 층수별 취약성
    # 근거: FEMA - 1층 침수 시 손실률 80%, 2층 30%, 3층 이상 10%

    ground_floors = building_info.get('지상층수', 1)

    if ground_floors == 1:
        floors_score = 100   # 1층 건물 - 극취약
    elif ground_floors == 2:
        floors_score = 70    # 2층 - 고취약
    elif ground_floors == 3:
        floors_score = 40    # 3층 - 중취약
    else:
        floors_score = 20    # 4층 이상 - 저취약 (1층만 침수)


    # 3-3. 건물 연식
    # 근거: 한국 건물손상연구(2017) - 30년 이상 건물은 내수성 50% 저하

    approval_date = building_info['사용승인일']
    ...
    if building_age >= 30:
        ...
        age_score = 30    # 5년 이하 - 신축
    else:
        age_score = 30 + ((building_age - 5) / 25) * 70


    # 취약성 통합
    # 근거: FEMA DDF(1992, 1200회) - 건물 유형이 가장 중요 (0.5)
    vulnerability_score = (
        (type_score * 0.50) +
        (floors_score * 0.30) +
        (age_score * 0.20)
...
| # | 데이터명 | 출처 | 필드명 | 비용 |
|:--|:--|:--|:--|:--|
| **7** | **건물 주용도** | 건축물대장 API | `mainPurpsCdNm` | 무료 |
| **8** | **지상층수** | 건축물대장 API | `grndFlrCnt` | 무료 |
...
**FEMA Depth-Damage Function (1992, 1200회 인용)**:[^3]

- 1층 주거시설 침수 1m 시: 손실률 **80%**
- 1층 상업시설 침수 1m 시: 손실률 **60%**
- 2층 이상 건물: 손실률 **30%** (1층만 침수)

**한국 건물손상연구 (2017)**:[^10]

- 30년 이상 노후 건물: 내수성 **50% 저하**
- 1층 단독주택 침수 50cm 시: 손실률 **75%** (한국 실측)
...
```python
"""
내륙 홍수(Fluvial Flooding) 리스크 평가 시스템
...
**결과**: 내륙 홍수 리스크 점수 (0-100점) 및 시나리오별 비교표.
