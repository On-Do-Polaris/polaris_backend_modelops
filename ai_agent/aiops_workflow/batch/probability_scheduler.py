...
class ProbabilityScheduler:
	...
	def __init__(self, config: Dict[str, Any]):
		self.config = config
		self.batch_processor = ProbabilityBatchProcessor(config.get('batch_config', {}))
	...
	def start(self, grid_coordinates_callback=None) -> None:
		...
		schedule = self.config.get('schedule', {'type': 'cron', 'month': 1, 'day': 1, 'hour': 2})
		...
		logger.info("ProbabilityScheduler started")
