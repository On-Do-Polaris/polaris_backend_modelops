'''
파일명: cold_wave_hscore_agent.py
최종 수정일: 2025-11-21
버전: v1
파일 개요: 극심한 한파 리스크 Hazard 점수(H) 산출 Agent
변경 이력:
	- 2025-11-21: v1 - H×E×V에서 H만 계산하도록 분리
		* Exposure, Vulnerability 계산 제거
		* 파일명: cold_wave_score_agent → cold_wave_hscore_agent
		* 클래스명: ColdWaveScoreAgent → ColdWaveHScoreAgent
'''
from typing import Dict, Any
from .base_hazard_hscore_agent import BaseHazardHScoreAgent


class ColdWaveHScoreAgent(BaseHazardHScoreAgent):
	"""
	극심한 한파 리스크 Hazard 점수(H) 산출 Agent
	기후 데이터 기반 한파일수 위험도 평가
	"""

	def __init__(self):
		"""
		ColdWaveHScoreAgent 초기화
		"""
