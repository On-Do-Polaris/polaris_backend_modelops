'''
파일명: water_scarcity_hscore_agent.py
최종 수정일: 2025-11-21
버전: v1
파일 개요: 물부족 리스크 Hazard 점수(H) 산출 Agent
변경 이력:
...
class WaterScarcityHScoreAgent(BaseHazardHScoreAgent):
	"""
	물부족 리스크 Hazard 점수(H) 산출 Agent
	"""
	...
	def calculate_hazard(self, collected_data: Dict[str, Any]) -> float:
		...
		risk_days = climate_data.get('water_scarcity_risk', 5)
