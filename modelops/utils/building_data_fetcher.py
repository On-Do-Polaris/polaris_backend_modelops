    ...
    def fetch_full_tcfd_data(self, lat: float, lon: float, address: str = None) -> Dict[str, Any]:
        ...
        if is_daedeok and len(basis_list) > 1:
            ...
            for bldg in basis_list:
                ...
                # 점수 계산 (층수, 면적 유사도)
                floor_score = 0
                if ground_floors == 4: floor_score += 50
                elif 3 <= ground_floors <= 5: floor_score += 30

                if underground_floors == 1: floor_score += 30
                elif underground_floors == 0: floor_score += 10

                area_score = 0
                if 12000 <= total_area <= 17000: area_score += 50  # 14558 ± 2500
                elif 10000 <= total_area <= 20000: area_score += 30
                elif total_area > 5000: area_score += 10

                year_score = 0
                if use_apr_date.startswith('2001'): year_score += 20
                elif use_apr_date.startswith('200'): year_score += 10
            ...
            if best_match:
                self.logger.info(f"선택된 건물: 지상{best_match.get('grndFlrCnt')}층, 지하{best_match.get('ugrndFlrCnt')}층, "
