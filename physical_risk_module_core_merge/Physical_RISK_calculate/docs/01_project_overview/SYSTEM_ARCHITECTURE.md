...
def calculate(self, lat: float, lon: float) -> Dict:
    return {
        ...
        'river_flood': self._calculate_river_flood_hazard(),
        'urban_flood': self._calculate_urban_flood_hazard(),
        'sea_level_rise': self._calculate_sea_level_rise_hazard(),
...
| SU25 | 극심한 고온일수 (일최고 ≥25°C) | 극심한 고온 |
| TR25 | 열대야일수 (일최저 ≥25°C) | 극심한 고온 |
| WSDIx | 극심한 고온 지속일수 | 극심한 고온 |
| TNn | 연간 최저기온 (°C) | 극심한 극심한 한파 |
| FD0 | 결빙일수 (일최저 <0°C) | 극심한 극심한 한파 |
| ID0 | 얼음일수 (일최고 <0°C) | 극심한 극심한 한파 |
| CSDIx | 극심한 한파 지속일수 | 극심한 극심한 한파 |
| RN | 연간 총 강수량 (mm) | 가뭄 |
| CDD | 최대 연속 무강수일수 | 가뭄 |
| SDII | 강수일 평균 강수강도 | 가뭄 |
...
3. 최종 리스크 점수 산출
   ├─ 극심한 고온: H(85) × E(90) × V(80) = 76.5점 (high)
   ├─ 하천 홍수: H(70) × E(75) × V(85) = 72.1점 (high)
...
|--------|-------------------|------------|
| 극심한 고온 | 100% | KMA SSP (TXx, SU25, TR25, WSDIx) |
| 극심한 극심한 한파 | 100% | KMA SSP (TNn, FD0, ID0, CSDIx) |
| 가뭄 | 90% | KMA SSP (CDD, SDII) + SMAP 토양수분 |
| 하천 홍수 | 100% | KMA SSP + 재난 API + DEM |
| 도시 홍수 | 100% | KMA SSP + 토지피복도 |
| 해수면 상승 | 100% | Haversine 거리 계산 |
| 태풍 | 70% | 통계 기반 (Best Track API 미연동) |
| 산불 | 100% | NDVI + 토지피복도 + KMA SSP |
| 물부족 | 70% | 인구 통계 (수도 사용량 API 미연동) |
...
# 4. 결과 출력
print(f"극심한 고온 리스크: {result['extreme_heat']['risk_score']}점")
