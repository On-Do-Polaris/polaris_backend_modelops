...
def calculate_coastal_hazard(scenario, target_year):
    ...
    근거:
    - IPCC AR6(2021): 해수면 상승이 해수면 상승의 직접 원인
...
**침수 가능성 (Inundation Probability)**:
해수면 상승에 따른 건물 침수 가능성을 평가하는 핵심 지표이다. 건물의 표고와 미래 해수면 고도의 차이를 기반으로 한다.

- 건물 표고 ≤ 미래 해수면: 100점 (완전 침수)
- 건물 표고 < (미래 해수면 + 1m): 80점 (높은 침수 위험)
- 건물 표고 < (미래 해수면 + 3m): 50점 (중간 침수 위험)
- 건물 표고 ≥ (미래 해수면 + 5m): 0점 (안전)

**해안선까지 거리 (Distance to Coastline)**:
건물과 해안선 간의 최단 거리를 측정하여 침수 노출 정도를 평가한다. 해안선에 가까울수록 해수면 상승으로 인한 피해 위험이 커진다.

- Critical (< 100m): 80점
- High (100m ~ 500m): 60점
- Medium (500m ~ 1000m): 40점
- Low (1000m 이상): 10점
...
```
국토지리정보원 DEM: https://map.ngii.go.kr
해양수산부 해양공간포털: https://www.msp.go.kr
```
...
$$
\text{취약성} = \text{Base}(10) + \text{Elevation\_Penalty} + \text{Basement\_Penalty}
$$
...
**해발고도 패널티 (Elevation_Penalty)**:
건물의 해발고도는 침수에 대한 가장 중요한 방어선이다. 고도가 낮을수록 해수면 상승 시 침수 위험이 급증한다.

- 해발 < 3m: +50점 (침수 확실시)
- 해발 < 5m: +30점 (고위험)
- 해발 ≥ 10m: -20점 (안전 고도)

**지하층 패널티 (Basement_Penalty)**:
지하층은 침수 시 심각한 구조적 손상과 복구 비용 증가를 야기한다. 특히 해발고도가 낮은 건물은 지하층 유무에 따라 취약성이 크게 달라진다.

- 해발 < 5m 이면서 지하층이 있는 경우: +20점
- 그 외: 0점
...
| # | 데이터명 | 출처 | 필드명 | 비용 |
|:--|:--|:--|:--|:--|
| **5** | **건물 위경도** | 건축물대장 API | `lat`, `lon` | 무료 |
| **6** | **건물 해발고도** | DEM | `elevation_m` | 무료 |
| **7** | **지하층수** | 건축물대장 API | `ugrndFlrCnt` | 무료 |
...
- **Nature 한국(2019, 35회 인용)**: 한국 해안 건물 노출 분석
- **World Bank 몰디브(2024, 최신)**: 해안 침수 정량 평가
- **NOAA 침수 매핑 가이드**: Modified Bathtub 방법론
...
- **FEMA Coastal Construction Manual (2011, 120회 인용)**: 해안 지역 건축물의 내수해 설계 표준.
- **국토교통부 (2019)**: 건축물의 피난·방화구조 등의 기준에 관한 규칙.
- **한국수자원공사 (2019)**: 해안 도시 침수 방어 시설 설계 기준.
...
```python
"""
해수면 상승(Sea Level Rise) 리스크 평가 시스템
...
def calculate_sea_level_rise_risk(building_info, scenario, target_year, dem=None, coastline_gdf=None):
    """
    최종 해수면 상승 리스크 계산
...
def main():
    ...
    for building in test_buildings:
        ...
        for scenario in scenarios:
            for year in years:
                result = calculate_sea_level_rise_risk(
    ...
    output_csv = 'sea_level_rise_risk_results.csv'
...
python sea_level_rise_assessment.py
```
...
# 작업 폴더 생성
mkdir sea_level_rise_project
cd sea_level_rise_project
```
...
sea_level_rise_risk_results.csv

# 엑셀로 열기
open sea_level_rise_risk_results.csv
```
...
def quick_test():
    ...
    print(f"해수면 상승 리스크: {risk:.1f}/100")
...
**결과**: 해수면 상승 리스크 점수 (0~100점) 및 시나리오별 비교표.[^6][^2][^12][^5][^9][^1]
