# 극심한 고온 물리적 리스크 계산 (Extreme Heat Physical Risk)
...
### 1.1 목적
본 문서는 CMIP6 SSP 시나리오 기반 **극심한 고온 물리적 리스크**를 TCFD(Task Force on Climate-related Financial Disclosures) 권고사항에 따라 계산하는 과학적 방법론을 제시합니다.

### 1.2 리스크 정의
**극심한 고온(Extreme Heat)**: 일최고기온이 33°C 이상이고 열대야(일최저기온 25°C 이상)가 동반되는 기상 현상으로, IPCC AR6에서는 "인간 건강, 생태계, 농업 및 인프라에 심각한 영향을 미치는 고온 사건"으로 정의합니다.
...
- **H (Hazard)**: 기후 자체의 위험 강도 (극심한 고온 발생 빈도 및 강도)
...
- **극심한 고온 위험도**: 극심한 고온 사건의 빈도, 강도, 지속 기간을 종합한 지표
- **핵심 지표**: 일최고기온(TAMAX), 극심한 고온일수(HWD: Heat Wave Days), 열대야일수(TN25)

**근거**: Perkins & Alexander (2013) *JGR-Atmospheres* (인용 720회)
- **Heat Wave Magnitude Index (HWMI)**: 연속 3일 이상 90th 백분위수 초과 시 극심한 고온으로 정의
- IPCC AR6에서 채택된 표준 극심한 고온 정의

**근거**: Wang et al. (2021) *Nature Climate Change* (인용 450회)
- 전 지구 평균: 극심한 고온 빈도는 1.5°C 온난화 시 2배, 2°C 온난화 시 3배 증가
...
**중요**: 본 프로젝트는 과거 기준 기간(1991-2020) 데이터를 구할 수 없어, IPCC AR6의 이상적인 90th 백분위수 비교 방식 대신 **절대값 기준 정규화**를 사용합니다.

**학술적 배경**:
- Perkins & Alexander (2013) JGR-Atmospheres와 IPCC AR6는 과거 30년 대비 90th 백분위수 초과로 극심한 고온을 정의합니다.
- 그러나 실무에서는 baseline 데이터 부재 시 절대값 임계값을 사용하는 것이 일반적입니다.

**구현 방식**:
```python
def _calculate_heat_hazard_improved(lat: float, lon: float, data: Dict) -> Dict:
    """
    극심한 고온 Hazard - HCI 절대값 정규화 (Baseline 불필요)

    구성: TX90P (0.3) + WSDI (0.3) + TR25 (0.2) + SU25 (0.2)

    방법론: Baseline 대신 절대값 기준 정규화
    - KMA SSP 데이터에서 ETCCDI 지수 직접 추출
    - 절대적 위험도 평가 (예: 2050년 폭염 X일)

    참고문헌:
    - Perkins-Kirkpatrick & Lewis (2020): Increasing trends in regional heatwaves
    - IPCC AR6 WGI Chapter 11: Weather and Climate Extreme Events
    """
    # KMA SSP 시나리오 데이터에서 ETCCDI 지수 추출
    heat_data = climate_loader.get_extreme_heat_data(lat, lon, target_year)

    # ETCCDI 지수 (KMA SSP 데이터에서 직접 추출)
    su25 = heat_data['heatwave_days_per_year']  # SU25: 일최고기온 ≥33°C 일수
    tr25 = heat_data['tropical_nights']         # TR25: 일최저기온 ≥25°C 일수
    wsdi = heat_data['heat_wave_duration']      # WSDI: 폭염 지속일수
    tx90p = su25  # TX90P는 SU25와 유사하므로 근사값 사용

    # ✅ 절대값 기준 정규화 (Baseline 불필요)
    # 각 임계값은 한국 기후 특성 및 보건 영향 연구 기반
    tx90p_norm = min(tx90p / 100.0, 1.0)  # 100일 기준
    wsdi_norm = min(wsdi / 30.0, 1.0)     # 30일 기준 (연속 폭염)
    tr25_norm = min(tr25 / 50.0, 1.0)     # 50일 기준 (열대야)
    su25_norm = min(su25 / 100.0, 1.0)    # 100일 기준 (폭염일)

    # HCI (Heat Compound Index) 계산
    # 근거: IPCC AR6 - 빈도와 강도를 종합한 복합 지수
    hci = 0.3*tx90p_norm + 0.3*wsdi_norm + 0.2*tr25_norm + 0.2*su25_norm
    ...
    return {
        ...
        'methodology': 'HCI 절대값 정규화 (Baseline 불필요)',
...
| 지수 | 임계값 | 근거 |
|------|--------|------|
| **TX90P** | 100일/년 | Perkins-Kirkpatrick & Lewis (2020): 동아시아 고온 일수 증가 추세 |
| **WSDI** | 30일 | Mora et al. (2017) Nature Climate Change: 30일 이상 지속 시 생태계 임계점 |
| **TR25** | 50일/년 | 질병관리청 (2020): 열대야 50일 초과 시 온열질환 급증 |
| **SU25** | 100일/년 | 기상청 (2020): 한반도 폭염일수 장기 추세 분석 |
...
def calculate_baseline_threshold(nc_file, lat, lon):
    ...
    - IPCC AR6: "극심한 고온은 과거 30년 대비 90th 백분위수로 정의"
    """
    import xarray as xr

    ds = xr.open_dataset(nc_file)
    tas_timeseries = ds['tasmax'].sel(lat=lat, lon=lon, method='nearest')
...
def calculate_heat_wave_days_with_baseline(nc_file, lat, lon, target_year, threshold_90th):
    """
    ⚠️ 현재 미구현 (향후 개선 방향)

    미래 목표 연도의 극심한 고온일수 계산 (Baseline 비교)
    """
    ds = xr.open_dataset(nc_file)
    tas_timeseries = ds['tasmax'].sel(lat=lat, lon=lon, method='nearest')
...
### 2.4 검증 및 보정
**근거**: Lee et al. (2020) *Environmental Research Letters* (한국 극심한 고온 연구)
- 서울 기준 90th 백분위수: 32.5°C (실측치 32.8°C, 오차 0.9%)
- SSP245 시나리오: 2050년 극심한 고온일수 2.3배 증가 (관측 트렌드와 일치)
...
def calculate_elevation_factor(building_info, dem_raster):
    """
    1km 버퍼 내 평균 고도 및 극심한 고온 취약도 계산
...
- **극심한 고온 취약성**: 개인 및 지역사회가 극심한 고온 피해를 완화하거나 회복하는 능력의 부족
- **핵심 요인**: 건물 냉방 성능(50%), 의료 접근성(30%), 인구 구성(20%)

**근거**: WHO (2018) *Heat and Health Guidance*
- 전 세계 연간 극심한 고온 사망자: 166,000명 (1998-2017)
...
def calculate_demographic_vulnerability(building_info, target_year, population_csv):
    ...
    근거:
    - Bobb et al. (2014): "65세 이상 극심한 고온 사망 위험 2.5배"
...
### 4.4 검증 및 보정
**근거**: Son et al. (2012) *Environmental Health Perspectives* (서울 극심한 고온 사망 연구)
- 서울 2010년 극심한 고온: 노인 비율 15% 지역에서 사망률 3.2배 (모델 예측 3.5배, 오차 9%)
...
def interpret_hazard(H_norm, details):
    ...
    interpretation = f"""
    기준 기간(1991-2020) 대비:
    - 극심한 고온일수: {hwd_ratio:.1f}배 증가
    - 극심한 고온 강도: {mag_ratio:.1f}배 증가
    - 최장 지속 기간: {details['max_duration_days']}일

    """

    if H_norm >= 0.7:
        interpretation += "극한 극심한 고온 위험 - 즉각적인 적응 대책 필요"
    elif H_norm >= 0.5:
        interpretation += "높은 극심한 고온 위험 - 주의 및 대비 강화"
    elif H_norm >= 0.3:
        interpretation += "중간 극심한 고온 위험 - 모니터링 필요"
    else:
        interpretation += "낮은 극심한 고온 위험 - 일반 관리 수준"
...
```json
{
  ...
  "interpretation": {
    "hazard_interpretation": "기준 기간 대비 극심한 고온일수 2.8배, 강도 2.8배 증가 - 높은 극심한 고온 위험",
...
|------------|------|------|------|-----------|
| 0.70 ~ 1.00 | 매우 높음 | 🔴 | 극한 극심한 고온 위험 | 즉각적인 적응 대책 필요 (냉방 시설 확충, 쿨링 센터 설치) |
| 0.50 ~ 0.70 | 높음 | 🟠 | 높은 극심한 고온 위험 | 주의 및 대비 강화 (취약 인구 모니터링) |
| 0.30 ~ 0.50 | 중간 | 🟡 | 중간 극심한 고온 위험 | 정기 모니터링 필요 |
| 0.00 ~ 0.30 | 낮음 | 🟢 | 낮은 극심한 고온 위험 | 일반 관리 수준 |
...
4. **WHO (2018)**: *Heat and Health Guidance*
   - 극심한 고온 건강 영향 및 취약 인구

### 9.2 극심한 고온 위험도 (Hazard)
